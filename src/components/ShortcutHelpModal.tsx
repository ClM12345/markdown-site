...
import { SHORTCUT_GROUPS } from '../data/shortcutHelp';
...
function KeyBadge({ keys }: { keys: string }) {
  ...
  return (
    <span className="flex items-center gap-1">
      {parts.map((p, i) => (
        <kbd key={i} className="inline-block min-w-[1.5rem] text-center px-1.5 py-0.5 text-xs font-mono rounded bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 shadow-sm">
          {p}
        </kbd>
      ))}
    </span>
...
export default function ShortcutHelpModal({ open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div
        className="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-lg max-h-[80vh] overflow-auto"
      ...
      >
        <div className="sticky top-0 bg-white dark:bg-gray-800 flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-sm font-bold text-gray-900 dark:text-white">快捷键说明</h2>
