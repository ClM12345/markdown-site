import { TEMPLATES } from '../data/markdownTemplates';
...
export default function TemplateModal({ open, onClose, onSelect, hasContent }: Props) {
  ...
  const handleSelect = (content: string) => {
    if (hasContent) {
      if (!window.confirm('当前编辑器已有内容，使用模板将覆盖现有内容。确认继续？')) return;
    }
    onSelect(content);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">模板中心</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">选择模板快速开始</p>
          </div>
        ...
        <div className="flex-1 overflow-auto p-4 grid sm:grid-cols-2 gap-3">
          {TEMPLATES.map((t) => (
            <button
              ...
              className="text-left p-4 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-500 hover:shadow-sm transition-all group"
            >
