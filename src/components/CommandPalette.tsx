...
export default function CommandPalette({ open, onClose, commands }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-start justify-center pt-[15vh]" onClick={onClose}>
      ...
      <div
        className="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden border border-gray-200 dark:border-gray-700"
      ...
      >
        <div className="flex items-center px-4 border-b border-gray-200 dark:border-gray-700">
          ...
          <input
            ...
            placeholder="输入命令..."
            className="w-full px-3 py-3 text-sm bg-transparent outline-none text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
          />
        </div>
        <div ref={listRef} className="max-h-72 overflow-auto py-1">
          {filtered.length === 0 && (
            <div className="px-4 py-6 text-center text-sm text-gray-400 dark:text-gray-500">无匹配命令</div>
          )}
          {filtered.map((cmd, i) => (
            <button
              key={cmd.id}
            ...
            >
              ...
              {cmd.shortcut && (
                <span className="shrink-0 ml-3 text-xs text-gray-400 dark:text-gray-500">{shortcutLabel(cmd.shortcut)}</span>
              )}
            </button>
          ))}
        </div>
